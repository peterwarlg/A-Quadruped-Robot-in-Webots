...
float t_trot=0.0;
float trot_para[5]=
{0.25,0.05,0.05,0.05,0.05};//trot参数：机身高，步高，左步长，右步长,速度
  ...
  for(int i=0;i<4;i++)
    {
      ...
      angel[i][1] = atan(foot_to_kuan[i][0]/(foot_to_kuan[i][1]/cos(angel[i][0])));
      angel[i][2] = 0.25-sqrt(pow(foot_to_kuan[i][0],2)+pow(foot_to_kuan[i][1],2)+pow(foot_to_kuan[i][2],2));
    }
}
  ...
  for(int i=0;i<4;i++)
    {
      for(int j=0;j<3;j++)
        {
        // if(j==2){wb_motor_set_position(servo[i][j], 0);}
